...
// Move libraries out of component scope
const libraries = ['geometry'];

const Map = ({ data, handleActiveMarker, active, setActive, range, location, setLocation }) => {
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (pos) => setLocation({ view: { lat: pos.coords.latitude, lng: pos.coords.longitude } }),
      () => setLocation({ view: { lat: 48.407326, lng: -123.329773 } })
    );
  }, [setLocation]);
