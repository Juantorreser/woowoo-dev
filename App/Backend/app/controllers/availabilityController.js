...
exports.createAvailability = async (req, res) => {
    ...
    try {
        ...
        const availability = {
            id: lastId !== 0 && lastId ? lastId + 1 : 1,
            healer: req.body.healer,
            service: req.body.service, // Added service field
            timeslots: req.body.timeslots, // Expecting an array here
            duration: req.body.duration,
            timezone: req.body.timezone || null,
            date: req.body.date,
...
// Retrieve all availabilities for a specific healer, date, and service
exports.findAllAvailability = async (req, res) => {
    const healerId = req.query.healer;
    const date = req.query.date;
    const service = req.query.service; // Added service query parameter
    ...
    try {
        const data = await Availability.findAll({
            where: {
                healer: healerId,
                date: date,
                service: service // Added service condition
            }
        });
        res.send(data);
    } catch (err) {
